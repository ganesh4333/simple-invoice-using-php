...
if(isset($id)){
	if ($type == 1) {
		if (isset($_GET['all_categories'])) {
			$items = $conn->query("SELECT i.*, p.description, p.id as pid, p.product as `name`, p.category_id as cid 
								   FROM invoices_items i 
								   INNER JOIN product_list p ON p.id = i.form_id");
		} else {
			$items = $conn->query("SELECT i.*, p.description, p.id as pid, p.product as `name`, p.category_id as cid 
								   FROM invoices_items i 
								   INNER JOIN product_list p ON p.id = i.form_id");
		...
		if (isset($_GET['all_categories'])) {
			$items = $conn->query("SELECT i.*, s.description, s.id as `sid`, s.`service` as `name`, s.category_id as cid 
								   FROM invoices_items i 
								   INNER JOIN service_list s ON s.id = i.form_id");
		} else {
			$items = $conn->query("SELECT i.*, s.description, s.id as `sid`, s.`service` as `name`, s.category_id as cid 
								   FROM invoices_items i 
								   INNER JOIN service_list s ON s.id = i.form_id");
...
<div class="card card-outline card-primary">
	...
	<h3 class="card-title"><?php echo !isset($_GET['id']) ? "New Estimation" :"Edit Estimation" ?></h3>
	</div>
	<div class="card-body">
		<div class="container-fluid">
			<form action="" id="invoice-form">
				...
				<div class="row">
					<div class="col-md-6">
						<div class="form-group">
							<label for="invoice_code" class="control-label">Estimation Code</label>
					...
					<div class="col-md-6">
						<div class="form-group">
							...
							<option value="1" <?php echo isset($type) && $type == 1 ? 'selected' : '' ?>>Inventory</option>
							<option value="2" <?php echo isset($type) && $type == 2 ? 'selected' : '' ?>>Projects</option>
							</select>
						</div>
					</div>
				</div>
				...
				<div class="row align-items-end">
					...
					<div class="col-md-3">
						<div class="form-group">
						<label for="form_id" class="control-label">Materials Required</label>
					...
					<div class="col-md-2">
						<div class="form-group">
							<label for="unit" class="control-label">Service Charges</label>
					...
					<div class="col-md-12">
						<table class="table table-bordered" id="item-list">
							...
							<tfoot>
								...
								<tr>
									<th class="text-right" colspan="4">Shipping Charges</th>
									<th><input type="text" id ="shipping" <?php echo isset($shipping) ? $shipping : ''; ?> onchange="calc_total()" name="shipping"></th>
									<th class="text-right" id="shipp" name="shipp"></th>
...
var item_arr = $.parseJSON('<?php echo json_encode($item_arr) ?>');
	...
	function calc_total(){
		var total = 0;
		var sh=document.getElementById('shipping').value;
		...
		var tax = (parseFloat(total) + parseFloat(sh))* parseFloat(tax_rate);
		var gtotal = parseFloat(tax) + parseFloat(total)+parseFloat(sh);
	...
	function add_item($obj = null){
		...
		start_loader();
			...
			total = parseFloat(price) * parseFloat(qty) + parseFloat(unit)
		}else{
			item_id = $obj.id
			qty = $obj.quantity;
			unit = $obj.unit;
			...
			total = parseFloat($obj.price) * parseFloat(qty) + parseFloat(unit)
		}
